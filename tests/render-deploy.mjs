...
const schema = `test_render_${Date.now()}`;
...
const env = {
  ...
  NODE_ENV: "production",
  RENDER: "true",
  RENDER_EXTERNAL_URL: "https://farm-deploy-test.onrender.com",
  APP_ORIGIN: "",
  SEED_EMAIL: "Render-Test@farm.local",
  SEED_PASSWORD: "RenderTestOnly!58319",
  PORT: "3012",
};
...
try {
  ...
  const login = await request(env.RENDER_EXTERNAL_URL);
  assert.equal(login.status, 200);
  assert.match(login.headers.get("set-cookie"), /; Secure/i);
  assert.match(login.headers.get("set-cookie"), /; HttpOnly/i);
  console.log(
    "PASS: fresh migration, seed twice, preserved password, rejected default password, public health, Render origin and Secure cookie",
  );
} finally {
  ...
  assert.match(schema, /^test_render_\d+$/);
